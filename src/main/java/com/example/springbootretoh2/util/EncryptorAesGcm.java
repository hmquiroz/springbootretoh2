...
public class EncryptorAesGcm {
    ...
    public static Map encriptar(String json) throws Exception{
        String cifradoJson  ="";
        ...
        byte[] encryptedText = EncryptorAesGcm.encryptWithPrefixIV(json.getBytes(UTF_8), secretKey, iv);
        cifradoJson =  CryptoUtils.hex(secretKey.getEncoded());
        System.out.println(String.format(OUTPUT_FORMAT, "Input (plain text)", json));
        System.out.println(String.format(OUTPUT_FORMAT, "Key (hex)", cifradoJson));
        result.put("input",json);
        result.put("secretKey",secretKey);
        result.put("iv",CryptoUtils.hex(iv));
        result.put("cifradojson",cifradoJson);
        return result;
    }

    public static String desencriptar(String hex,SecretKey secrectKeyInput,String ivInput) throws Exception{
        String descifradoJson  ="";
        String OUTPUT_FORMAT = "%-30s:%s";
        // encrypt and decrypt need the same key.
        // get AES 256 bits (32 bytes) key
        SecretKey secretKey = secrectKeyInput;

        // encrypt and decrypt need the same IV.
        // AES-GCM needs IV 96-bit (12 bytes)
        byte[] iv = ivInput.getBytes();
        byte[] encryptedText = EncryptorAesGcm.encryptWithPrefixIV(hex.getBytes(UTF_8), secretKey, iv);

        System.out.println(String.format(OUTPUT_FORMAT, "Input (hex)", CryptoUtils.hex(encryptedText)));

        descifradoJson = EncryptorAesGcm.decryptWithPrefixIV(encryptedText, secretKey);

        System.out.println(String.format(OUTPUT_FORMAT, "Decrypted (plain text)", descifradoJson));
        return descifradoJson;
    }
